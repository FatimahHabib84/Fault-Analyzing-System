<?php
    ...
    if  (isset($_POST['add'])) {
        ...
        $sql = "INSERT INTO `equipment` (`equipmentNo`, `plantNo`) VALUES ('  $equipmentNo', '$plantNo');";
        $result = mysqli_query($con,$sql);
        if($result){
            header("location: addNewEquip.php?status2=1");
            exit();
        }
        else{
            header("location: addNewEquip.php?status2=0");
            exit();
        }
    }
?>
<?php if(isset($_GET['status2']) and $_GET['status2']==1){ ?>
        <h1>Successfull</h1>
        <?php } else if(isset($_GET['status2']) and $_GET['status2']==0){ ?> 
        <h1>Sorry, this equipment is already exist</h1>
        <?php } else { ?>
    <div class="wrapper">
    <form name="form"  method="POST" action="addNewEquip.php">
            <table>
                <tr>
